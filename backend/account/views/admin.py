...
class UserAdminAPI(APIView):
    ...
    def post(self, request):
        ...
        for user_data in data:
            ...
            user = User(username=user_data[0], password=make_password(user_data[1]), email=user_data[2], is_approved=True)
...
class GenerateUserAPI(APIView):
    ...
    def post(self, request):
        ...
        for number in range(data["number_from"], data["number_to"] + 1):
            ...
            # Admin-created users are automatically approved
            user = User(username=f"{data['prefix']}{number}{data['suffix']}", password=make_password(raw_password), is_approved=True)
